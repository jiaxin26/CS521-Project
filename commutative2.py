...
class CommutativePass:
    ...
    def __call__(self, module):
        ...
        # Find all reduce_prod nodes
        for node in graph.nodes:
            if not (any(node.target == pattern for pattern in self.reduce_prod_patterns) and len(node.args) >= 1):
                continue
            ...
            if (isinstance(arg, torch.fx.Node) and 
                arg.op == "call_function" and 
                any(arg.target == pattern for pattern in self.exp_patterns)):
                ...
                with graph.inserting_before(node):
                    ...
                    reduce_sum = graph.call_function(torch.sum, args=(exp_arg,), kwargs=kwargs)
                    # Then compute Exp(ReduceSum(A))
                    new_node = graph.call_function(torch.exp, args=(reduce_sum,))
...
def test_commutative():
    ...
    try:
        ...
        print("Optimized matches original:", torch.allclose(original_output, expected_output))
