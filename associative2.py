...
class SqrtAssociativePass:
    ...
    def __call__(self, module):
        ...
        for node in graph.nodes:
            if not (any(node.target == pattern for pattern in self.mul_patterns) and len(node.args) >= 2):
                continue
            ...
            # Check if lhs is a multiplication with sqrt
            if (isinstance(lhs, torch.fx.Node) and 
                lhs.op == "call_function" and 
                any(lhs.target == pattern for pattern in self.mul_patterns) and
                len(lhs.args) == 2):
                
                lhs_lhs, lhs_rhs = lhs.args[0], lhs.args[1]
                
                # Check if lhs_rhs is a sqrt
                if (isinstance(lhs_rhs, torch.fx.Node) and 
                    lhs_rhs.op == "call_function" and 
                    lhs_rhs.target == torch.sqrt):
                    
                    sqrt_arg = lhs_rhs.args[0]
                    
                    # Check if rhs is a multiplication with sqrt
                    if (isinstance(rhs, torch.fx.Node) and 
                        rhs.op == "call_function" and 
                        any(rhs.target == pattern for pattern in self.mul_patterns) and
                        len(rhs.args) == 2):
                        
                        rhs_lhs, rhs_rhs = rhs.args[0], rhs.args[1]
                        
                        # Check if rhs_lhs is a sqrt of the same argument
                        if (isinstance(rhs_lhs, torch.fx.Node) and 
                            rhs_lhs.op == "call_function" and 
                            rhs_lhs.target == torch.sqrt and
                            rhs_lhs.args[0] == sqrt_arg):
                            
                            # Create new multiplication chain: A⊙B⊙C
                            with graph.inserting_before(node):
                                # First multiply A and B
                                ab = graph.call_function(torch.mul, args=(lhs_lhs, sqrt_arg))
                                # Then multiply with C
                                new_node = graph.call_function(torch.mul, args=(ab, rhs_rhs))
                            
                            # Replace the original node with the new one
                            node.replace_all_uses_with(new_node)
                            
                            # Add nodes to delete set
                            nodes_to_delete.update([node, lhs, rhs, lhs_rhs, rhs_lhs])
